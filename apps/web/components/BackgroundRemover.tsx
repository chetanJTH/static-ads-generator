...
export default function BackgroundRemover() {
  ...
  const handleRemoveBackground = async () => {
    if (!originalFile || isProcessingRef.current) return
    ...
    try {
      ...
      // Don't update currentImage here to avoid triggering useEffect
      // The canvas will be updated via the cutoutBase64 state
    } catch (error) {
  ...
  useEffect(() => {
    ...
    // Use cutoutBase64 if available, otherwise use currentImage
    const imageToShow = cutoutBase64 || currentImage
    setupCanvas(canvas, imageToShow, state === 'processing_bg')
  }, [currentImage, cutoutBase64, state])
